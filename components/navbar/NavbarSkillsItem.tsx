...
function NavbarSkillsItem() {
  ...
  return (
    <button ref={buttonRef} className={`relative navbar-item-pointer-up ${itemActive ? "navbar-item-pointer-up-show" : "navbar-item-pointer-up-hide"}`} onClick={() => setItemActive(prev => !prev)}>
        ...
        <div className={`absolute top-[150%] left-0 md:right-0 md:left-auto p-5 bg-light-1 w-[300px] sm:w-[500px] ${itemActive ? "opacity-100" : "opacity-0"} transition-all duration-300 z-10 pointer-events-none rounded-lg`}>
